...
import br.com.dio.exception.InvestmentAccountNotFoundException;
...
public class InvestmentRepository {
    ...
    public InvestmentWallet initInvestment(final AccountWallet account, final long id) {
        if (!wallets.isEmpty()) {
            List<Object> accountsInUse = wallets.stream()
    ...
    public Investment findById(final long id) {
        return investments.stream()
                .filter(a -> a.id() == id)
                .findFirst()
                .orElseThrow(() -> new InvestmentAccountNotFoundException("O investimento '" + id + "' não foi encontrado"));
    }
