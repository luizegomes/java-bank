...
public class AccountRepository {
    ...
    public AccountWallet create(final List<String> pix, final long initialFunds) {
        if (!accounts.isEmpty()) {
            List<String> pixUse = accounts.stream()
    ...
    private void checkFundsTransaction(final AccountWallet source, final long amount) {
    ...
    public Map<LocalDate, List<MoneyAudit>> getHistory(String pix) {
        AccountWallet wallet = accounts.stream()
                .filter(a -> a.getPix().contains(pix))
                .findFirst()
                .orElseThrow(() -> new AccountNotFoundException("Conta com Pix '" + pix + "' não encontrada."));

        return wallet.getFinancialTransactions().stream()
                .collect(Collectors.groupingBy((Function<MoneyAudit, LocalDate>) audit -> audit.createdAt().toLocalDate()));
    }
